...
1. **補 S5、M 兩面通關徽章**——Canva AI 生圖額度用盡（生到第 10 張）。醫師另有帳號 `philia81301@jessie.com`，但**換帳號要由使用者在 Claude 設定中斷再重新連接 connector，Agent 端無法切換**。補生規格：金色圓牌＋月桂葉＋綠緞帶＋角色頭像，存成 `design/assets-quiz/s5-badge.png`、`m-badge.png`，重跑 build 即自動換上
